...
public sealed class HomeControllerLoginProviderTests
{
    ...
    // CasLogin finishes with LocalRedirect, which would throw on an off-site target after a good
    // sign-in, so the ReturnUrl handed to CAS must already be local.
    [Fact]
    public void Login_CasOnly_OffSiteReturnUrl_FallsBackToAppRoot()
    {
        var controller = CreateController(LoginProviders.Cas);

        var result = Assert.IsType<RedirectResult>(controller.Login("https://evil.example.com/"));

        Assert.DoesNotContain("evil.example.com", Uri.UnescapeDataString(result.Url));
    }
    ...
    private static HomeController ArrangeForLogout(LoginProviders enabled, string authenticationMethod)
    {
