...
public class HomeControllerCasUrlTests
{
    ...
    [Fact]
    public void Login_PathLookingLikeApi_IsNotTreatedAsApi()
    {
        // "/apiary" shares a prefix with "/api" but is not under it.
        var controller = CreateController("secure-test.vetmed.ucdavis.edu", pathBase: "/2");

        var result = Assert.IsType<RedirectResult>(controller.Login("/2/apiary/hives"));
    ...
    [Fact]
    public void Login_ApiReturnUrl_WhenBasePathPrefixesIt_ReturnsUnauthorized()
    {
        // A base that is a character prefix of "/api" must not be stripped off "/api/...",
        // or the guard chops the URL into something it no longer recognizes as an API path.
        var controller = CreateController("secure-test.vetmed.ucdavis.edu", pathBase: "/a");

        Assert.IsType<UnauthorizedResult>(controller.Login("/api/students/dvm"));
    }

    [Fact]
    public void Login_BareApiReturnUrl_ReturnsUnauthorized()
    {
        var controller = CreateController("secure-test.vetmed.ucdavis.edu", pathBase: "/2");

        Assert.IsType<UnauthorizedResult>(controller.Login("/2/api"));
    }
    ...
    {
        // "~/api/..." resolves against the PathBase and reaches the same endpoint as
        // "/api/...", so it must not slip past the guard.
        var controller = CreateController("secure-test.vetmed.ucdavis.edu", pathBase: "/2");
    ...
    [Fact]
    public void Login_AppRelativePathLookingLikeApi_IsNotTreatedAsApi()
    {
        var controller = CreateController("secure-test.vetmed.ucdavis.edu", pathBase: "/2");
