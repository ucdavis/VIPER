...
public class PublicUrlServiceTests
{
    ...
    public void BaseUrl_Unconfigured_NoRequest_FallsBackToLocalDevelopmentOrigin()
    {
        ...
        // The origin is resolved once into a static readonly field, so setting the variable here
        // would be too late to affect it. Mirror the production rule instead, which keeps the
        // expectation right for a missing, non-numeric or out-of-range value alike.
        string? httpsPort = Environment.GetEnvironmentVariable("ASPNETCORE_HTTPS_PORT");
        int expectedPort = int.TryParse(httpsPort, out int parsed) && parsed > 0 && parsed < 65536 ? parsed : 7157;
    ...
    [InlineData("https://viper.vetmed.ucdavis.edu/2?")]
    [InlineData("https://viper.vetmed.ucdavis.edu/2#")]
