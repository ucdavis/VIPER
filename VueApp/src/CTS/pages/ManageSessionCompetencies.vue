...
const compAddUpdate = ref({
    sessionCompetencyId: null,
    sessionId: sessionId,
    competencyId: null,
    order: null,
    levelIds: [],
    roleId: null,
}) as Ref<SessionCompetencyAddUpdate>
...
function getSession() {
    get(apiUrl + "cts/courses/" + courseId + "/sessions/" + sessionId).then((r) => (session.value = r.result))
}

function getSessionCompetencies() {
    get(apiUrl + "cts/courses/" + courseId + "/sessions/" + sessionId + "/competencies").then(
        (r) => (sessionCompetencies.value = r.result),
    )
}
...
function getComps() {
    get(apiUrl + "cts/competencies").then((r) => (allComps.value = r.result))
}

function getLevels() {
    get(apiUrl + "cts/levels?course=true").then((r) => (levels.value = r.result))
}

//search/filter in comp search box
function filterComps(val: any, update: any) {
    update(() => {
        const srch = val.toLowerCase()
        filteredComps.value = allComps.value.filter(
    ...
    if (multiRole.value) {
        ...
        for (var role of roles.value) {
            ...
            if (compAddUpdate.value.levelIds.length) {
                ...
                var roleResult = await postOrPut(
                    apiUrl + "cts/courses/" + courseId + "/sessions/" + sessionId + "/competencies",
                    compAddUpdate.value,
                )
                success = success && roleResult.success
            }
        }
    } else {
        var r = await postOrPut(
            apiUrl + "cts/courses/" + courseId + "/sessions/" + sessionId + "/competencies",
            compAddUpdate.value,
        )
        success = r.success
...
function selectComp(sc: SessionCompetency) {
    ...
    if (multiRole.value) {
        ...
        compAddUpdateRolesAndLevels.value = compValues.reduce<string[]>((result, c) => {
            c.levels?.forEach((level) => result.push(c.roleId + "-" + level.levelId))
            return result
        }, [])
    } else {
        //get all level ids currently selected for this comp
        const firstCompValue = compValues[0]
        compAddUpdate.value.levelIds = firstCompValue?.levels
            ? firstCompValue.levels.reduce<number[]>((result, l) => {
                  result.push(l.levelId)
                  return result
              }, [])
            : []
    }
        ...
        >
            ...
            <q-form @submit="submitSessionComp">
                <div class="row">
                    <div class="col-12">
                        <q-select
                            ...
                            outlined
                            label="Competency"
                            v-model="compAddUpdate.competencyId"
                            :readonly="compAddUpdate.sessionCompetencyId != null"
                            :options="allComps"
                ...
                >
                    ...
                    >
                        <q-checkbox
                            v-model="compAddUpdate.levelIds"
                            :label="l.levelName"
                            :val="l.levelId"
                        ></q-checkbox>
                    </div>
                ...
                <template v-if="multiRole">
                    <div
                        class="row items-center roleLevelSelect"
                        v-for="r in roles"
                        :key="r.roleId"
                    >
                        <div class="col">
                        ...
                        >
                            <q-checkbox
                                v-model="compAddUpdateRolesAndLevels"
                                :label="l.levelName"
                                :val="r.roleId + '-' + l.levelId"
                            ></q-checkbox>
                        </div>
                    </div>
                </template>
                ...
                <tbody>
                    <template v-if="showLegacyComps">
                        ...
                        >
                            ...
                            <template
                                v-for="level in ['Know', 'Obs', 'Perf', 'Assess']"
                                :key="level"
                            >
                                <td>
                                    <q-icon
                    ...
                    <tr
                        v-for="c in sessionCompetencies"
                        :key="c.sessionCompetencyId"
                    >
                        <td>{{ c.competencyNumber }} {{ c.competencyName }}</td>
                        <td v-if="multiRole">{{ c.roleName }}</td>
                        <template
                            v-for="level in ['Knowledge', 'Observed', 'Performed', 'Assessed']"
                            :key="level"
                        >
