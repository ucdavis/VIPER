...
async function loadAudit(page: number, perPage: number, sortBy: string, descending: boolean) {
    ...
    get(u.toString()).then(({ result, pagination: resultPagination }) => {
        auditRows.value = result.map((o: any) => {
            try {
...
async function loadModifiers() {
    const { get } = useFetch()
    get(baseUrl + "audit/modifiers").then(({ result }) => (modifiers.value = result))
}

async function loadUsers() {
    const { get } = useFetch()
    get(apiUrl + "people?active=true").then(({ result }) => (allUsers.value = result))
}

async function loadActions() {
    const { get } = useFetch()
    get(baseUrl + "audit/actions").then(({ result }) => (actions.value = result))
}

async function loadAreas() {
    const { get } = useFetch()
    get(baseUrl + "audit/areas").then(({ result }) => (areas.value = result))
}
