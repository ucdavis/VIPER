<script setup lang="ts">
    ...
    import type { Competency, Level, Milestone, MilestoneLevel, MilestoneLevelUpdate } from '@/CTS/types'
    ...
    async function load() {
        ...
        competencies.value = competencies.value.filter(c => !milestones.value.some(m => m.competencyId == c.competencyId))
        ...
        if (competencyId.value > 0) {
            ...
            if (comp != undefined) {
                ...
                if (r.success) {
                    ...
                    let result = await post(apiUrl + "cts/bundles/" + r.result.bundleId + "/competencies", bundleCompetency)
                    get(apiUrl + "cts/milestones").then(r => milestones.value = r.result)
                    get(apiUrl + "cts/competencies").then(r => competencies.value = r.result)
                }
            }
...
<template>
    ...
    <q-form @submit="createMilestone">
        <div class="row items-center">
            <div class="col-12 col-md-6 col-lg-4">
                <q-select dense options-dense outlined label="Select Competency"
                          :options="competencies" :option-label="(opt) => opt.number + ' ' + opt.name" option-value="competencyId"
                          v-model="competencyId" emit-value map-options>
                </q-select>
            </div>
            <div class="col q-ml-md">
                <q-btn icon="add" label="Add Milestone" type="submit" color="green"></q-btn>
            </div>
        </div>
    </q-form>
    ...
    <div class="row">
        <q-select dense options-dense outlined label="Select Milestone" class="col-12 col-md-6"
        ...
        <q-form @submit="submitLevels">
            <div class="row" v-for="milestoneLevel in milestoneLevels">
                ...
                <div class="col-12 col-sm-6 col-lg-6">
                    <q-editor v-model="milestoneLevel.description" outlined label="Description"
                              :toolbar="[
