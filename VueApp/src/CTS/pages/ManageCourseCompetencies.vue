<script setup lang="ts">
    ...
    const { get, put, createUrlSearchParams } = useFetch()
    ...
    async function getCourses() {
        if (selectedTerm.value?.termCode) {
            ...
            if (courseId) {
                var c = courses.value.find(c => c.courseId == courseId)
                if (c != undefined) {
    ...
    function getCourseRoles() {
        get(apiUrl + "cts/courses/" + selectedCourse.value.courseId + "/roles")
            ...
            .then(() => courseRolesUpdate.value = courseRoles.value.reduce<number[]>((result: number[], v: Role) => { result.push(v.roleId); return result }, []))
    }
...
<template>
    ...
    <div v-if="showRolesForm" class="q-mb-md">
        <q-form @submit="updateCourseRoles">
            <div v-for="r in roles">
                <q-checkbox v-model="courseRolesUpdate"
                            :val="r.roleId"
                            :label="r.name">
                </q-checkbox>
            </div>
            <q-btn dense no-caps label="Update Roles" type="submit" class="q-px-md" color="primary"></q-btn>
        </q-form>
    </div>

    <div v-if="selectedCourse != null" class="q-mb-md">
        <h4>
            Roles
            <q-btn dense outline no-caps label="Manage" color="secondary" icon="edit" @click="showRolesForm = true" class="q-px-md"></q-btn>
        </h4>
        <div v-if="courseRoles.length">
            <div v-for="cr in courseRoles">
        ...
        <table cellspacing="0" cellpadding="3">
            ...
            <tbody>
                <tr v-for="s in sessions">
                    ...
                    <td>{{ s.title }}</td>
                    <td>
                        <RouterLink :to="'ManageSessionCompetencies?courseId=' + selectedCourse.courseId + '&sessionId=' + s.sessionId">
                            {{ s.competencyCount }}
                        </RouterLink>
                    </td>
                </tr>
