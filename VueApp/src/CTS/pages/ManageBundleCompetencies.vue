...
const bundleId =
    route.query.bundleId === undefined || route.query.bundleId === null ? 0 : parseInt(route.query.bundleId.toString())
...
function loadBundleComps() {
    ...
    get(apiUrl + "cts/bundles/" + bundleId + "/groups").then(
        (r) => (bundleCompetencyGroups.value = [defaultGroup, ...r.result]),
    )
}
...
function filterComps(val: any, update: any, _abort: any) {
    update(() => {
        const srch = val.toLowerCase()
        competencyOptions.value = competencies.value.filter(
            (v) => (v.number.toLowerCase() + " " + v.name.toLowerCase()).indexOf(srch) > -1,
        )
    })
}
...
async function addBundleCompetency() {
    bundleCompetency.value.order =
        bundleCompetencies.value.filter(
            (bc) => bc.bundleCompetencyGroupId === bundleCompetency.value.bundleCompetencyGroupId,
        ).length + 1
...
async function updateBundleCompetency() {
    var result = await put(
        apiUrl + "cts/bundles/" + bundleId + "/competencies/" + bundleCompetency.value.bundleCompetencyId,
...
function editGroup(group: BundleCompetencyGroup) {
...
<template>
            ...
            <q-form @submit="saveBundleCompetencyGroup()">
                <div class="row">
                    <q-input
                        ...
                        v-model="bundleCompetencyGroup.name"
                    ></q-input>
                </div>
                <div class="row">
                    <q-input
                        ...
                        v-model="bundleCompetencyGroup.order"
                    ></q-input>
                </div>
                <div class="row q-mt-sm">
                    ...
                    <q-btn
                        type="button"
                        @click="clearBundleCompetencyGroup()"
                        color="secondary"
                        dense
                        class="col-5 col-sm-2 col-md-1 q-px-sm q-mr-md"
                        label="Cancel"
        ...
        >
            <div class="row">
                <q-select
                    ...
                    v-model="bundleCompetency.competencyId"
                    emit-value
                    map-options
                ...
                <q-select
                    ...
                    label="Group"
                    v-model="bundleCompetency.bundleCompetencyGroupId"
                    emit-value
                    map-options
                    :options="bundleCompetencyGroups"
                    option-value="bundleCompetencyGroupId"
                    option-label="name"
                ></q-select>
                <q-btn
                    dense
        ...
        <q-card class="q-pa-lg">
            <q-form @submit="updateBundleCompetency()">
                <q-card-section>
                    <div class="row">
                        <div class="col-12">
                            {{ editingBundleCompetency?.competencyNumber }}
                            {{ editingBundleCompetency?.competencyName }}
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12">
                            <q-select
                                ...
                                options-dense
                                outlined
                                v-model="bundleCompetency.roleId"
                                label="Role"
                                :options="roles"
                                option-value="roleId"
                                option-label="name"
                    ...
                    <div class="row">
                        <div class="col-12">
                            <q-select
                                ...
                                options-dense
                                outlined
                                v-model="bundleCompetency.levelIds"
                                multiple
                                label="Levels"
                                :options="levels"
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-4">
                            <q-input
                                ...
                                dense
                                outlined
                                v-model="bundleCompetency.order"
                                label="Order"
                            ></q-input>
                        </div>
                    </div>
    ...
    <q-table
        ...
        :key="group.bundleCompetencyGroupId ?? 'default'"
    >
        <template #top>
        ...
        <template #body-cell-competencyname="props">
            <q-td :props="props"> {{ props.row.competencyNumber }} {{ props.row.competencyName }} </q-td>
        </template>
        <template
            v-for="level in levels"
            :key="level.levelId"
            #[`body-cell-level_${level.levelId}`]="props"
        >
            <q-td :props="props">
                <q-icon
