<script setup lang="ts">
    ...
    const studentClassYear = ref({ studentClassYearId: 0, personId: 0, ross: false, leftReason: null, leftTerm: null, comment: "", active: false }) as Ref<StudentClassYearUpdate>
    ...
    async function getStudents() {
        ...
        if (classYear?.value && classYear.value.value) {
            ...
            let u = apiUrl + "students/dvm/?classYear=" + classYear.value.value + "&includeAllClassYears=" + !activeOnly.value
            const r = await get(u)
            ...
            problems.result.forEach((p : any) => {
                var s = allStudents.value.find(s => s.personId == p.personId)
                if (s != undefined) {
    ...
    function clear() {
        studentClassYear.value= { studentClassYearId: 0, personId: 0, ross: false, leftReason: null, leftTerm: null, comment: "", active: false, classYear: null }
        showForm.value = false
    }

    function selectStudent(student: StudentClassYearProblem, cy: StudentClassYear) {
    ...
    async function load() {
        ...
        get(apiUrl + "students/dvm/leftReasons")
            .then(data => {
                reasons.value = data.result
                    .map((r: any) => ({ label: r.reason, value: r.classYearLeftReasonId }))
            })
...
<template>
    ...
    <q-dialog v-model="showForm" @hide="clear">
        <q-card style="width: 500px; max-width: 80vw;">
            <q-form @submit="submitStudentClassYear" v-model="studentClassYear">
                ...
                <q-card-actions align="evenly">
                    ...
                    <q-btn no-caps label="Delete" type="button" padding="xs md" @click="deleteStudentClassYear" color="red"></q-btn>
                </q-card-actions>
            </q-form>
        </q-card>
    </q-dialog>
        ...
        <div class="row q-mb-md">
            ...
            <div class="col col-md-2 col-lg-2 offset-md-3">
                <q-btn v-if="classYear.value" :label="'Import students into ' + classYear?.label" dense no-caps color="secondary"
                       :to="'StudentClassYearImport?classYear=' + classYear.value"></q-btn>
            </div>
        </div>
    </q-form>
    ...
    <q-table :rows="students"
        ...
        <template v-slot:body-cell-avatar="props">
            <q-td :props="props">
                <q-avatar square>
                    <img :src="viperUrl + 'public/utilities/getbase64image.cfm?altphoto=1&mailId=' + props.row.mailId" class="smallPhoto" />
                </q-avatar>
            </q-td>
        </template>
