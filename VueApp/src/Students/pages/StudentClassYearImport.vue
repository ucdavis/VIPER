...
async function getStudentsByTermAndClassLevel() {
    ...
    if (term.value?.value && classLevel.value?.value) {
        students.value = (
            await get(apiUrl + "students/dvm/byTermAndClassLevel/" + term.value.value + "/" + classLevel.value.value)
        ).result
        noStudentsFound.value = students.value.length === 0
    }
}
async function importStudents() {
    await post(apiUrl + "students/dvm/" + classYear.value + "/import", selectedStudents.value)
    getStudentsByTermAndClassLevel()
    selectedStudents.value = []
}
function selectAll() {
    selectedStudents.value = students.value.reduce((ids, s) => {
        if (s.classYears.length === 0) ids.push(s.personId)
        return ids
    }, [] as number[])
}
    ...
    <q-form>
        <div class="row q-mb-md">
            ...
            <q-btn
                ...
                @click="getStudentsByTermAndClassLevel()"
            ></q-btn>
            <q-btn
                label="Return to class list"
                dense
                no-caps
                class="q-ml-xl q-px-sm"
                color="secondary"
                :href="'StudentClassYear?classYear=' + classYear"
            ></q-btn>
        </div>
        ...
        <div v-if="students.length">
            ...
            <q-btn
                ...
                class="q-px-md"
                color="primary"
                label="Import Selected Students"
                ...
                >
                    <q-item-section side>
                        <q-checkbox
                            ...
                            :val="student.personId"
                            :disable="student.classYears.length !== 0"
                        ></q-checkbox>
                    </q-item-section>
                    ...
                    <q-item-section>
                        <q-item-label> {{ student.lastName }}, {{ student.firstName }} </q-item-label>
                    </q-item-section>
        ...
        <div v-if="noStudentsFound">No students found</div>
    </q-form>
</template>
