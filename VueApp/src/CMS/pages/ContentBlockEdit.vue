...
// Delete the dialog's uploads and forget them, for the two ways unsaved edits get thrown away:
// a confirmed discard on route leave, and the reload after a save conflict.
async function discardDialogUploads() {
    await rollbackFiles(dialogUploadGuids.splice(0))
}

// Prompt before leaving with unsaved edits, matching the Effort forms' guard.
onBeforeRouteLeave(async () => {
    const confirmed = await confirmClose()
    if (confirmed) await discardDialogUploads()
    return confirmed
})
