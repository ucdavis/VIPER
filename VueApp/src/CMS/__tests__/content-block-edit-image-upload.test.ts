 ...
 * dialog uploads when the user discards their edits on route leave. RichTextEditor is stubbed so
 * only the props ContentBlockEdit passes it are under test (the dialog behind them has its own
 * tests). Mock ViperFetch; capture vue-router's onBeforeRouteLeave callback to drive it.
 */
...
// Stubbed down to the props this page supplies, so a change to the editor's internals can't fail
// these tests; rich-text-editor.test.ts covers what the real component does with them.
...
// Defaults to the edit route for block 7; pass empty params for the create-mode mount.
async function mountEdit(routeArgs?: { params?: Record<string, string> }) {
    const router = createTestRouter()
    await router.push({ name: "CmsContentBlockEdit", params: routeArgs?.params ?? { id: "7" } })
