...
/**
 * Where `text`'s non-blank content begins and ends within `[from, to)`. Each bound is clamped to the
 * opposite one so an all-whitespace slice collapses rather than inverting the range.
 */
function trimmedBounds(text: string, from: number, to: number): [number, number] {
    const slice = text.slice(from, to)
    const lead = slice.length - slice.trimStart().length
    const trail = slice.length - slice.trimEnd().length
    return [Math.min(to, from + lead), Math.max(from, to - trail)]
}
...
function trimRange(range: Range) {
    ...
    if (end.nodeType === Node.TEXT_NODE) {
        ...
        const [, stop] = trimmedBounds(text, start === end ? range.startOffset : 0, range.endOffset)
        range.setEnd(end, stop)
    }
    if (start.nodeType === Node.TEXT_NODE) {
        const text = start.textContent ?? ""
        // range.endOffset is read after the setEnd above, so the two sides can't cross.
        const [begin] = trimmedBounds(text, range.startOffset, start === end ? range.endOffset : text.length)
        range.setStart(start, begin)
    }
}
