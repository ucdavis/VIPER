...
// Every mount attaches to the body (see mountEditor), and QDialog leaves its teleported content
// there too, so clear it between tests.
afterEach(() => {
...
// The link type select, driven the way a user's pick does: v-model updates `kind` and the
// component's own listener clears the address.
async function pickLinkKind(wrapper: Awaited<ReturnType<typeof mountEditor>>, kind: LinkKind) {
    wrapper.findComponent(EditorLinkDialog).findComponent({ name: "QSelect" }).vm.$emit("update:modelValue", kind)
    await nextTick()
}

test("changing the link type clears the address and each type checks its own format", async () => {
    const wrapper = await mountEditor({ toolbar: FULL_TOOLBAR })
    const editor = await openViaHandler(wrapper, "link")
    const dialog = wrapper.findComponent(EditorLinkDialog).vm as unknown as LinkDialogVm

    dialog.address = "ucdavis.edu/vetmed"
    await nextTick()

    // Carrying the address across a type change built hrefs like mailto:https://ucdavis.edu/vetmed,
    // a link that silently fails for every reader.
    await pickLinkKind(wrapper, "email")
    expect(dialog.address).toBe("")

    // And typing the wrong shape into the field is caught before it reaches the href.
    dialog.address = "https://ucdavis.edu"
    await nextTick()
    await submitDialog(wrapper)
    expect(editor.vm.runCmd).not.toHaveBeenCalled()

    dialog.address = "vetmed-help@ucdavis.edu"
    await nextTick()
    await submitDialog(wrapper)
    expect(editor.vm.runCmd).toHaveBeenCalledWith(
        "insertHTML",
        '<a href="mailto:vetmed-help@ucdavis.edu">vetmed-help@ucdavis.edu</a>',
    )
})

test("a web address is held to the shared protocol allowlist, relative links included", async () => {
    const wrapper = await mountEditor({ toolbar: FULL_TOOLBAR })
    const editor = await openViaHandler(wrapper, "link")
    const dialog = wrapper.findComponent(EditorLinkDialog).vm as unknown as LinkDialogVm

    // An unrecognized scheme comes out of normalizeHref as the nonsense "https://javascript:alert(1)"
    // rather than rejected, so nothing downstream ever says no to one.
    // oxlint-disable-next-line no-script-url -- a javascript: URL is the unsafe input under test here
    dialog.address = "javascript:alert(1)"
    await nextTick()
    await submitDialog(wrapper)
    expect(editor.vm.runCmd).not.toHaveBeenCalled()

    // Relative links are the reason this field can't reuse Link Collections' absolute-only rule.
    dialog.address = "/2/CMS/ManageFiles#top"
    await nextTick()
    await submitDialog(wrapper)
    expect(editor.vm.runCmd).toHaveBeenCalledWith(
        "insertHTML",
        '<a href="/2/CMS/ManageFiles#top">/2/CMS/ManageFiles#top</a>',
    )
})

test("a phone link takes any separator style but not an address that has no number in it", async () => {
    const wrapper = await mountEditor({ toolbar: FULL_TOOLBAR })
    const editor = await openViaHandler(wrapper, "link")
    const dialog = wrapper.findComponent(EditorLinkDialog).vm as unknown as LinkDialogVm

    await pickLinkKind(wrapper, "phone")
    dialog.address = "vetmed-help@ucdavis.edu"
    await nextTick()
    await submitDialog(wrapper)
    expect(editor.vm.runCmd).not.toHaveBeenCalled()

    // Counting digits rather than matching a format is what keeps an extension usable here.
    dialog.address = "+1 (530) 752-1360 ext 4"
    await nextTick()
    await submitDialog(wrapper)
    expect(editor.vm.runCmd).toHaveBeenCalledWith(
        "insertHTML",
        '<a href="tel:+1 (530) 752-1360 ext 4">+1 (530) 752-1360 ext 4</a>',
    )
})
...
test("the table dialog inserts the shape it collected and rejects one past the builder's clamp", async () => {
    const wrapper = await mountEditor({ toolbar: FULL_TOOLBAR })
    const editor = await openViaHandler(wrapper, "table")
    const dialog = wrapper.findComponent(EditorTableDialog).vm as unknown as TableDialogVm
