 ...
 */
function toRelativeViperUrl(value: string, origin: string = globalThis.location.origin): string {
    const trimmed = value.trim()
 ...
 */
function isViperUrl(value: string, origin: string = globalThis.location.origin): boolean {
    try {
...
function parseLinkHref(href: string): { kind: LinkKind; address: string } {
...
function buildImageHtml(opts: { src: string; alt: string; origin?: string }): string {
...
// Exported so the dialog rejects what this builder would otherwise silently clamp.
