import type { RotationScheduleData } from '../services/RotationService'
...
export function useScheduleStateUpdater() {
   ...
   */
  function findWeekInSemesters(
    semesters: SemesterWithWeeks[],
    weekId: number
  ): WeekWithSchedules | null {
  ...
  function addScheduleToWeek(
    ...
    if (!scheduleData.schedulesBySemester) return
    ...
    // Add clinician to recent list if not already there
    if (!scheduleData.recentClinicians.some(c => c.mothraId === newSchedule.mothraId)) {
   ...
   */
  function removeScheduleFromWeek(
    scheduleData: RotationScheduleData,
    scheduleId: number
  ): boolean {
    if (!scheduleData.schedulesBySemester) return false
  ...
  function updateSchedulePrimaryStatus(
    ...
    if (isPrimary) {
      schedules.forEach((s: ScheduleEntry) => {
        if (s.instructorScheduleId !== scheduleId) {
