    ...
    public class PublicUrlService : IPublicUrlService
    {
        ...
        public string BaseUrl
        {
            get
            {
                ...
                string fromRequest = FromRequest(_httpContextAccessor.HttpContext?.Request);
                return fromRequest.Length > 0 ? fromRequest : LocalDevelopmentOrigin();
            }
        }
        ...
        /// sent from a background job. Deployed environments never reach it because
        /// PublicUrlOptionsValidator fails startup when the canonical origin is missing.
        /// </summary>
        private static string LocalDevelopmentOrigin()
        {
            string httpsPort = System.Environment.GetEnvironmentVariable("ASPNETCORE_HTTPS_PORT") ?? "7157";
            return int.TryParse(httpsPort, out int port) && port > 0 && port < 65536
