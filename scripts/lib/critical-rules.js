...
/**
 * SonarAnalyzer security rule patterns - SPECIFIC security-related rules only
 * Based on actual vulnerability/security rule analysis, not broad ranges
 */
const SONAR_SECURITY_PATTERNS = [
    // Injection and traversal vulnerabilities
    { pattern: /^S20(77|83|91)$/, category: "Injection & Traversal" }, // SQL injection, path traversal, XPath injection
    // Authentication and session security
    { pattern: /^S2092$/, category: "Session Security" }, // Cookie security
    // Cryptography and random generation
    { pattern: /^S(2245|4426|4507|4423)$/, category: "Cryptography" }, // Weak random, weak crypto, debug enabled, weak SSL
    // Advanced injection and security vulnerabilities
    { pattern: /^S(5042|5122|5131|5144|5443|5659|5693|5738|5766|5856)$/, category: "Advanced Security" },
    // Modern security (JWT, etc.)
    { pattern: /^S(6781|6641|5883|5147)$/, category: "Modern Security" }, // JWT secrets, connection injection, OS injection, NoSQL injection
]
 ...
 * @param {string} ruleId - The rule identifier (e.g., 'S2083', 'CA5351')
 * @returns {boolean} - True if it's a .NET security rule
 */
function isDotNetSecurityRule(ruleId) {
    // SonarAnalyzer security rules (pattern-based detection)
    if (ruleId.startsWith("S")) {
        return SONAR_SECURITY_PATTERNS.some((pattern) => pattern.pattern.test(ruleId))
    }

    // Microsoft CA security rules (range-based detection)
    if (ruleId.startsWith("CA")) {
        const ruleNumber = Number.parseInt(ruleId.slice(2), 10)
        if (!Number.isNaN(ruleNumber)) {
            return CA_SECURITY_RANGES.some((range) => ruleNumber >= range.min && ruleNumber <= range.max)
        }
    }
...
function categorizeRule(ruleId) {
    ...
    if (isCriticalSecurityRule(ruleId) || isDotNetSecurityRule(ruleId)) {
