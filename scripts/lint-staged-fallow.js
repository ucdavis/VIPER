...
function normalizeIssuePath(issuePath) {
    ...
    // fallow runs with cwd: VUEAPP_DIR and emits paths relative to that cwd.
    // Resolve against VUEAPP_DIR (not process.cwd()) so issues can be matched
    // against the staged-files set regardless of where the script is invoked from.
    const abs = path.resolve(VUEAPP_DIR, issuePath)
...
if (!report) {
    // runFallow already logged the underlying reason. In blocking mode (CI,
    // npm run lint) fail closed so a crashed fallow doesn't silently pass.
    // In non-blocking mode (default pre-commit) keep the original behavior:
    // a fallow crash shouldn't block the developer's commit.
    process.exit(shouldBlockOnWarnings() ? 1 : 0)
}
