...
function runJscpd(scanDir, jsonOutPath, minLines, formatFlags) {
    ...
    if (!fs.existsSync(reportPath)) {
        logger.error(`jscpd did not produce a report at ${reportPath} (exit ${result.status ?? "?"}).`)
        return null
    }
...
let totalFindings = 0
let jscpdFailed = false

function scanWith(label, scanDir, scopedSet, minLines, formatFlags, groupLabel) {
    logger.plain(`🔎 JSCPD — scanning ${label} (${scopedSet.size} staged file(s) in scope)`)
    const groups = withTempDir((tmp) => {
        const report = runJscpd(scanDir, tmp, minLines, formatFlags)
        if (report === null) {
            jscpdFailed = true
            return []
        }
...
if (tsFiles.size > 0) {
    scanWith("VueApp/src", VUEAPP_SRC, tsFiles, TS_MIN_LINES, [], "Clone groups involving staged VueApp files")
}
...
if (csFiles.size > 0) {
    scanWith(
        ...
        csFiles,
        CS_MIN_LINES,
        ["--format", "csharp", "--pattern", "**/*.cs"],
        "Clone groups involving staged C# files",
    )
}
